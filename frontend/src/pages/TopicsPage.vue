  ...
  <div class="page-container">
    ...
    <div class="form-section mb-6">
      <div class="flex flex-wrap gap-3">
        ...
        <div
          class="px-6 py-3 rounded-full cursor-pointer transition-all duration-200 text-lg font-medium"
          :class="activeTopicId === null ? 'bg-gray-800 text-white shadow-lg' : 'bg-white text-gray-700 hover:shadow-md border-2 border-gray-200'"
        ...
        >
          ...
          <el-tag
            size="large"
            class="ml-2"
            :style="{ backgroundColor: activeTopicId === null ? 'rgba(255,255,255,0.3)' : '#33333320', color: activeTopicId === null ? 'white' : '#333' }"
          >
            {{ uncategorizedCount }}
          </el-tag>
        </div>
    ...
    >
      <div v-if="currentExcerpt" class="space-y-4">
        ...
        <div>
          ...
          <p class="text-base text-gray-700 leading-relaxed">{{ currentExcerpt.contentSummary }}</p>
        </div>
        ...
        >
          ...
          <p class="text-lg text-gray-800 leading-relaxed">{{ currentExcerpt.elderlyNotes }}</p>
        </div>
      </div>
    ...
    >
      ...
      <div v-else class="space-y-4">
        ...
        >
          ...
          <p class="text-base text-gray-700 mb-2">{{ version.content }}</p>
...
const uncategorizedCount = computed(() => {
  return allExcerpts.value.filter(e => !e.topicId).length
})
...
const filterExcerpts = () => {
  if (activeTopicId.value === null) {
    excerpts.value = allExcerpts.value.filter(e => !e.topicId)
  } else {
    excerpts.value = allExcerpts.value.filter(e => e.topicId === activeTopicId.value)
  }
}
...
const openCategorizeDialog = (excerpt: ProgramExcerpt) => {
  currentExcerpt.value = excerpt
  selectedTopicId.value = excerpt.topicId
